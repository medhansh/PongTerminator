...
const Pong: React.FC = () => {
  ...
  useEffect(() => {
    const initAudio = async () => {
      try {
        ...
        // Set them in the audio store
        // Note: The audio store expects these to be set, but we'll handle playing directly
  ...
  const updateAI = useCallback(() => {
    ...
    if (Math.abs(diff) > 5) {
      const aiSpeed = PADDLE_SPEED * 0.8; // Slightly slower than player
      if (diff > 0) {
  ...
  }, [ballX, ballY, ballSpeedX, ballSpeedY, playerY, aiY, updateBall, setBallSpeed, incrementPlayerScore, incrementAiScore]);
  ...
  }, [gameState, updateAI, updateBallPhysics, playerY, aiY, ballX, ballY]);
